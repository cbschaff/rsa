...
class LunarLander(gym.Env, EzPickle):
    ...
    def __init__(self):
        ...
        ob_dim = N_OBS_DIM + 22 if self.terrain_in_ob else N_OBS_DIM
    ...
    def reset(self):
        ...
        self.terrain_x = chunk_x
        self.terrain_y = smooth_y
    ...
    def step(self, action):
        ...
        if self.terrain_in_ob:
            for x, y in zip(self.terrain_x, self.terrain_y):
                state.append((x - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2))
                state.append((y - (self.helipad_y+LEG_DOWN/SCALE)) / (VIEWPORT_H/SCALE/2))
        assert len(state)==N_OBS_DIM + 22 * self.terrain_in_ob
        self.curr_step += 1

        reward = 0
        dx = pos.x - self.target_cx
        dy = pos.y - self.target_cy
        shaping = \
            - 100*np.sqrt(dx*dx + dy*dy)/2 \
        ...
        at_target = np.sqrt(dx*dx + dy*dy) < TARGET_R/SCALE

        done = self.game_over or oob or at_target or timeout
        success = False
        crash = False
        if done:
            if self.game_over or oob:
                reward -= 100
                crash = True
                self.lander.color1 = (1.0,0.0,0.0)
            elif at_target:
                success = True
                reward += 100
                self.lander.color1 = (0.0,1.0,0.0)
            if self.human_test:
                self.render()
                import time
                time.sleep(1.0)
    ...
    def render(self, mode='human'):
        ...
        self.viewer.draw_circle(TARGET_R / SCALE, 20, color=[255, 0, 0], filled=False, linewidth=5).add_attr(t)
...
class LunarLanderReacher(LunarLander):
    continuous = True
