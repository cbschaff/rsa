...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        # Remove distance to goal from reward shaping. CHANGED
        else:
            shaping = \
                - 100*np.sqrt(state[2]*state[2] + state[3]*state[3]) \
                - 100*abs(state[4]) + 10*state[6] + 10*state[7]   # And ten points for legs contact, the idea is if you
        ...
        reward -= m_power*FUEL_COST_MAIN
        reward -= s_power*FUEL_COST_SIDE
        ...
        if done:
            ...
            elif ((not timeout and at_site) or landed) and self.goal_seeking:
                success = True
                reward += 100
...
class LunarLanderNoGoal(LunarLander):
    goal_seeking = False

class LunarLanderContinuousNoGoal(LunarLander):
    continuous = True
    goal_seeking = False
